...
describe('UserRepositoryImplement Integration Test', () => {
  ...
  describe('Transaction Support', () => {
    let transactionRollbackResult: UserEntity[];
    let transactionSuccessResult: UserEntity[];
    ...
    beforeAll(async () => {
      ...
      const transactionTestData = MOCK_USER_DATA.transaction.test;
      try {
        await db.transaction(async (tx) => {
          const transactionAggregate =
            userFactory.createAggregate(transactionTestData);
          await repository.create({
            user: transactionAggregate,
            transaction: tx,
          });
      ...
      transactionRollbackResult = await db.query.users.findMany({
        where: (user, { eq }) => eq(user.email, transactionTestData.email),
      });

      // Test success transaction
      const transactionSuccessData = MOCK_USER_DATA.transaction.success;
      await db.transaction(async (tx) => {
        const successAggregate = userFactory.createAggregate(
          transactionSuccessData,
        );
        await repository.create({ user: successAggregate, transaction: tx });
      });
      transactionSuccessResult = await db.query.users.findMany({
        where: (user, { eq }) => eq(user.email, transactionSuccessData.email),
      });
    });

    it('should rollback transaction on error', () => {
      // Assert - User should not exist due to rollback
      expect(transactionRollbackResult).toHaveLength(NUMBERS.ZERO);
    });

    it('should commit transaction on success', () => {
      // Assert - User should exist
      expect(transactionSuccessResult).toHaveLength(NUMBERS.ONE);
      expect(transactionSuccessResult[NUMBERS.ZERO].name).toBe(
        MOCK_USER_DATA.transaction.success.name,
      );
    });
  });
});
